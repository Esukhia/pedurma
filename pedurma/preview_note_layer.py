...
def parse_note(note, walker, page_content, plus_present):
    ...
    if plus_present:
        plus_note = note.replace('+', '\+')
        if re.search(rf"\S+་([^#]\S+་?){plus_note}", page_content):
            note_pattern = re.search(rf"\S+་([^#]\S+་?){plus_note}", page_content)
            last_syl, note_match = get_last_syl_and_note_match(note_pattern.group(1), plus_note)
        ...
        if note_pattern.group(1):
            ...
            if re.search(rf"\S+་([^#]\S+་?){note}", page_content):
                ...
                last_syl, note_match = get_last_syl_and_note_match(note_pattern.group(1), note)
...
def parse_page(page, note_annotation, char_walker):
    ...
    for note in notes:
        ...
        if match:
            if match.group(2):
                note_info, page_content = parse_note(note, char_walker, page_content, True)
        else:
...
def update_hybird_pecha_note_layer(preview_text, hybird_pecha_path, vol_num):
    ...
    note_yml_path = Path(hybird_pecha_path / f"{hybird_pecha_path.stem}.opf" / "layers" / f"v{vol_num:03}" / "PedurmaNote.yml")
