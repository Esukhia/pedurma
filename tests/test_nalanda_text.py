...
def test_save_page():
    ...
    base_path = Path('./tests/data/page_with_note/')
    project_name = "nalanda"
    note_pg_link = "https://iiif.bdrc.io/bdr:I1PD95846::I1PD958460231.jpg/full/max/0/default.jpg"
    # create empty page
    (base_path / project_name / f"{text_id}/pages/{page_id}.txt").write_text("", encoding='utf-8')
    #creat empty note
    empty_pg_note = get_empty_note(note_pg_link, vol=1)
    (base_path / project_name / f"{text_id}/notes/{page_id}.yml").write_text(empty_pg_note, encoding='utf-8')
    pedurma = Pedurma(project_name, base_path)
    ...
    (base_path / project_name / f"{text_id}/pages/{page_id}.txt").unlink()
    (base_path / project_name / f"{text_id}/notes/{page_id}.yml").unlink()
